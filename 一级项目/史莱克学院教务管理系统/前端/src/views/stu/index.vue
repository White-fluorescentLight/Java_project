...
const queryAllClazz = async () => {
  ...
  if (result.code) clazzs.value = result.data
}
...
const updateStu = async (id) => {
  ...
  if (result.code) stu.value = result.data
}
...
const delById = async (id) => {
  ElMessageBox.confirm('确认删除此数据?', '删除学生', {
...
const delByIds = async () => {
  ElMessageBox.confirm('确认删除选中数据?', '批量删除', {
    confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning'
  }).then(async () => {
    const result = await deleteApi(selectIds.value.join(','))
    if (result.code) {
      ElMessage.success('删除成功')
      queryPage()
    } else {
...
const openViolation = (id) => {
  ElMessageBox.prompt('请输入违纪扣分', '提示', {
    ...
    inputPattern: /\d?/, inputErrorMessage: '数据无效'
  }).then(async (val) => {
